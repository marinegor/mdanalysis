...
class BaseAuxReaderTest(object):
    ...
    def test_time_non_constant_dt(self, reader):
        ...
        with pytest.raises(
            ValueError, match="If dt is not constant, must have a valid time selector"
        ):
            reader.time
...
def assert_auxstep_equal(A, B):
    ...
    if A.step != B.step:
        raise AssertionError(
            "A and B refer to different steps: A.step = {}, " "B.step = {}".format(
                A.step, B.step
            )
        )
    if A.time != B.time:
        raise AssertionError(
            "A and B have different times: A.time = {}, " "B.time = {}".format(
                A.time, B.time
            )
        )
    if isinstance(A.data, dict):
        ...
        if any(A.data != B.data):
            # e.g. XVGReader
            raise AssertionError(
                "A and B have different data: A.data = {}, " "B.data = {}".format(
                    A.data, B.data
                )
            )
