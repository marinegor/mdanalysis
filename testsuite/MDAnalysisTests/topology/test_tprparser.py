...
class TestTPR(TPRAttrs):
    ...
    ref_moltypes = np.array(
        ["AKeco"] * 214 + ["SOL"] * 11084 + ["NA+"] * 4, dtype=object
    )
...
def _test_is_in_topology(name, elements, topology_path, topology_section):
    ...
    for element in elements:
        assert (
            element in getattr(top, topology_section).values
...
def test_all_bonds(topology, bond):
    ...
    bond_type_in_topology = functools.partial(
        _test_is_in_topology, topology_section="bonds"
    )
...
def test_all_angles(topology, angle):
    angle_type_in_topology = functools.partial(
        _test_is_in_topology, topology_section="angles"
    )
...
def test_all_dihedrals(topology, dih):
    dih_type_in_topology = functools.partial(
        _test_is_in_topology, topology_section="dihedrals"
    )
...
@pytest.mark.parametrize("topology", BONDED_TPRS)
@pytest.mark.parametrize(
    "impr", (("IDIHS", [(2, 3, 4, 5), (3, 4, 5, 6)]), ("PIDIHS", [(5, 6, 7, 8)]))
)
def test_all_impropers(topology, impr):
    impr_type_in_topology = functools.partial(
        _test_is_in_topology, topology_section="impropers"
    )
