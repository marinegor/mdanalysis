...
class Testrmsd(object):
    ...
    def test_p_frames(self, p_first, p_last):
        ...
        assert (
            p_first.universe.trajectory.ts.frame != p_last.universe.trajectory.ts.frame
        )
    ...
    def test_weights_and_superposition_1(self, u):
        ...
        weighted = rms.rmsd(
            u.trajectory[0], u.trajectory[1], weights=weights, superposition=True
        )
        firstCoords = rms.rmsd(u.trajectory[0], u.trajectory[1], superposition=True)
...
class TestRMSD(object):
    ...
    def test_rmsd_atomgroup_selections(self, universe, client_RMSD):
        # see Issue #1684
        R1 = MDAnalysis.analysis.rms.RMSD(universe.atoms, select="resid 1-30").run(
            **client_RMSD
        )
    ...
    def test_custom_weighted_list(self, universe, correct_values_mass, client_RMSD):
        ...
        RMSD = MDAnalysis.analysis.rms.RMSD(universe, weights=list(weights)).run(
            step=49, **client_RMSD
        )
        assert_almost_equal(
            RMSD.results.rmsd,
    ...
    def test_rmsd_backbone_and_group_selection(
        ...
        RMSD = MDAnalysis.analysis.rms.RMSD(
            ...
            reference=universe,
            select="backbone",
            groupselections=["backbone and resid 1:10", "backbone and resid 10:20"],
        ).run(step=49, **client_RMSD)
        assert_almost_equal(
            RMSD.results.rmsd,
    ...
    def test_ref_mobile_mass_mismapped(
        ...
        RMSD = MDAnalysis.analysis.rms.RMSD(
            universe, reference=reference, select="all", weights="mass", tol_mass=100
        )
        RMSD.run(step=49, **client_RMSD)
        assert_almost_equal(
            RMSD.results.rmsd,
