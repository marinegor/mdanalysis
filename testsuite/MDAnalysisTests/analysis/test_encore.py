...
class TestEncore(object):
    ...
    def test_triangular_matrix(self):
        ...
        triangular_matrix_2 = encore.utils.TriangularMatrix(
            size=size, loadfile=filename
        )
        assert_equal(
...
inconsistent results",
    ...
    @pytest.mark.xfail(os.name == "nt", reason="Not yet supported on Windows.")
    ...
    def test_rmsd_matrix_with_superimposition(self, ens1):
        ...
        for i, rmsd in enumerate(reference.results.rmsd):
            assert_allclose(
                conf_dist_matrix[0, i], rmsd[2], rtol=0, atol=1.5e-3, err_msg=err_msg
            )
    ...
    def test_ensemble_superimposition(self):
        ...
        align.AlignTraj(
            aligned_ensemble1, aligned_ensemble1, select="name CA", in_memory=True
        ).run()
        aligned_ensemble2 = mda.Universe(PSF, DCD)
        align.AlignTraj(
            aligned_ensemble2, aligned_ensemble2, select="name *", in_memory=True
        ).run()
    ...
    def test_covariance_matrix(self, ens1):
        ...
        covariance = encore.covariance.covariance_matrix(
            ens1,
    ...
    def test_hes(self, ens1, ens2):
        ...
        result_value = results[0, 1]
        min_bound = 1e5
        assert result_value > min_bound, (
            "Unexpected value for Harmonic "
            "Ensemble Similarity: {0:f}. Expected {1:f}.".format(
                result_value, min_bound
            )
        )
    ...
    def test_hes_align(self, ens1, ens2):
        ...
        assert_allclose(
            ...
            rtol=0,
            atol=1.5e3,
            err_msg="Unexpected value for Harmonic Ensemble Similarity: {0:f}. Expected {1:f}.".format(
    ...
    def test_ces_to_self(self, ens1):
        ...
        assert_allclose(
            ...
            rtol=0,
            atol=1.5e-7,
            err_msg="ClusteringEnsemble Similarity to itself not zero: {0:f}".format(
                result_value
            ),
        )
    ...
    def test_ces(self, ens1, ens2):
        ...
        assert_allclose(
            ...
            rtol=0,
            atol=1.5e-2,
            err_msg="Unexpected value for Cluster Ensemble Similarity: {0:f}. Expected {1:f}.".format(
                result_value, expected_value
            ),
        )
    ...
    def test_dres_to_self(self, ens1):
        ...
        assert_allclose(
            ...
            rtol=0,
            atol=1.5e-2,
            err_msg="Dim. Reduction Ensemble Similarity to itself not zero: {0:f}".format(
                result_value
            ),
        )
    ...
    def test_dres(self, ens1, ens2):
        ...
        assert result_value < upper_bound, (
            "Unexpected value for Dim. "
            "reduction Ensemble Similarity: {0:f}. Expected {1:f}.".format(
                result_value, upper_bound
            )
        )
    ...
    def test_dres_without_superimposition(self, ens1, ens2):
        ...
        assert_allclose(
            ...
            rtol=0,
            atol=1.5e-1,
            err_msg="Unexpected value for Dim. reduction Ensemble Similarity: {0:f}. Expected {1:f}.".format(
                result_value, expected_value
            ),
        )

    def test_ces_convergence(self, ens1):
        expected_values = [0.3443593, 0.1941854, 0.06857104, 0.0]
    ...
    def test_dres_convergence(self, ens1):
        ...
        try:
            ...
            # the test 10 times:
            warnings.warn(
                message="Test 'test_dres_convergence' failed, "
...
class TestEncoreClustering(object):
    ...
    def test_clustering_one_ensemble(self, ens1):
        ...
        expected_value = 7
        assert len(cluster_collection) == expected_value, (
            "Unexpected " "results: {0}".format(cluster_collection)
        )
    ...
    def test_clustering_two_methods(self, ens1):
        ...
        assert len(cluster_collection[0]) == len(
            cluster_collection[1]
        ), "Unexpected result: {0}".format(cluster_collection)
    ...
    def test_clustering_AffinityPropagationNative_direct(self, ens1):
        ...
        expected_value = 7
        assert (
            len(set(cluster_assignment)) == expected_value
    ...
    def test_clustering_KMeans_direct(self, ens1):
        ...
        assert (
            len(set(cluster_assignment)) == clusters
        ), "Unexpected result: {0}".format(cluster_assignment)
    ...
    def test_clustering_two_different_methods(self, ens1):
        ...
        cluster_collection = encore.cluster(
            [ens1],
            method=[
                encore.AffinityPropagation(preference=-7.5, random_state=0),
    ...
    def test_clustering_method_w_no_distance_matrix(self, ens1):
        ...
        assert len(cluster_collection) == 10, "Unexpected result: {0}".format(
            cluster_collection
        )
    ...
    def test_clustering_two_methods_one_w_no_distance_matrix(self, ens1):
        pytest.importorskip("sklearn")
        cluster_collection = encore.cluster(
            [ens1], method=[encore.KMeans(17), encore.AffinityPropagationNative()]
        )
        assert len(cluster_collection[0]) == len(
    ...
    def test_ClusterCollection_get_ids(self, cc):
        assert_equal(
            cc.get_ids(), [0, 1, 2], err_msg="ClusterCollection ids aren't as expected"
        )
    ...
    def test_metadata_size_error(self):
        error_message = (
            'Size of metadata having label "label" is '
...
class TestEncoreDimensionalityReduction(object):
    ...
    def test_dimensionality_reduction_different_method(self, ens1, ens2):
        ...
        dimension = 3
        coordinates, details = encore.reduce_dimensionality(
            [ens1, ens2], method=encore.PrincipalComponentAnalysis(dimension=dimension)
        )
        assert_equal(
            coordinates.shape[0],
            dimension,
    ...
    def test_dimensionality_reduction_two_methods(self, ens1, ens2):
        ...
        coordinates, details = encore.reduce_dimensionality(
            [ens1, ens2],
            method=[
                encore.StochasticProximityEmbeddingNative(dims[0]),
