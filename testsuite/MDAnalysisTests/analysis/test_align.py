...
class TestGetMatchingAtoms(object):
    ...
    def test_nomatch_residues_raise_empty(
        self, universe, reference_small, strict, selection="protein and backbone"
    ):
...
class TestAlign(object):
    ...
    def test_AlignTraj_outfile_default_exists(self, universe, reference, tmpdir):
        ...
        with mda.Writer(
            str(tmpdir.join("rmsfit_align_test.dcd")), n_atoms=fitted.atoms.n_atoms
        ) as w:
    ...
    def test_AlignTraj_weighted(self, universe, reference, tmpdir):
        ...
        self._assert_rmsd(
            reference, fitted, -1, 6.929083032629219, weights=universe.atoms.masses
        )
    ...
    def test_AlignTraj_custom_mass_weights(self, universe, reference, tmpdir):
        outfile = str(tmpdir.join("align_test.dcd"))
        x = align.AlignTraj(
            universe, reference, filename=outfile, weights=reference.atoms.masses
        ).run()
        fitted = mda.Universe(PSF, outfile)
        assert_allclose(x.results.rmsd[0], 0, rtol=0, atol=1.5e-3)
        assert_allclose(x.results.rmsd[-1], 6.9033, rtol=0, atol=1.5e-3)
    ...
    def test_AlignTraj_partial_fit(self, universe, reference, tmpdir):
        ...
        align.AlignTraj(
            universe, reference, select="resid 1-20", filename=outfile, weights="mass"
        ).run()
    ...
    def _assert_rmsd(self, reference, fitted, frame, desired, weights=None):
        ...
        rmsd = rms.rmsd(
            reference.atoms.positions, fitted.atoms.positions, superposition=True
        )
        assert_allclose(
            rmsd,
            desired,
...
class TestAverageStructure(object):
    ...
    def test_average_structure_deprecated_attrs(self, universe, reference):
        ...
        with pytest.warns(DeprecationWarning, match=wmsg):
            assert_equal(
                avg.universe.atoms.positions, avg.results.universe.atoms.positions
            )
...
class TestAlignmentProcessing:
    ...
    def test_fasta2select_ClustalW(self, tmpdir):
        ...
        sel = align.fasta2select(
            self.seq, is_aligned=False, alnfilename=alnfile, treefilename=treefile
        )
