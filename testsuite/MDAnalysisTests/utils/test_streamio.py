...
class TestNamedStream_filename_behavior(object):
    ...
    @pytest.mark.parametrize(
        "func",
        (abspath, basename, dirname, expanduser, normpath, relpath, split, splitext),
    )
    ...
    def test_expandvars(self):
        ...
        assert_equal(
            value, reference, err_msg="os.path.expandvars() did not expand HOME"
        )
...
class TestStreamIO(RefAdKSmall):
    ...
    def test_XYZReader(self, streamData):
        u = MDAnalysis.Universe(
            streamData.as_NamedStream("XYZ_PSF"), streamData.as_NamedStream("XYZ")
        )
