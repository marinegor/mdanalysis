...
class TestTransTable(object):
    ...
    def test_s2r_1d(self, tt):
        for sidx, rix in zip(
            [[0, 1], [1, 0], [1, 1]], [[0, 3, 1, 2], [1, 2, 0, 3], [1, 2, 1, 2]]
        ):
    ...
    def test_lazy_building_SR(self, tt):
        ...
        SR = tt.SR
        assert_rows_match(
            tt.SR, np.array([np.array([0, 3]), np.array([1, 2]), None], dtype=object)
        )
...
class TestLevelMoves(object):
    ...
    def test_move_residue(self, u):
        ...
        assert res not in dest.residues
        ...
        assert res not in source.residues
    ...
    def test_move_residuegroup_single_segment(self, u):
        ...
        for r in res:
            assert r not in source.residues
...
class TestTopologyGuessed(object):
    ...
    def test_guessed(self, names, types, resids, resnames, bonds, top):
        ...
        assert names not in guessed
        assert resids not in guessed
    ...
    def test_read(self, names, types, resids, resnames, bonds, top):
        ...
        assert types not in read
        assert resnames not in read
