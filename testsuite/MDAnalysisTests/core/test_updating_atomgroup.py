...
class TestUpdatingSelection(object):
    ...
    def ag_updating_compounded(self, u, ag):
        return u.select_atoms("around 2 group sele", sele=ag, updating=True)
    ...
    def test_compounded_update(self, u, ag_updating_compounded):
        ...
        assert_equal(ag_updating_compounded.indices, target_idxs0)
        next(u.trajectory)
    ...
    def test_chained_update2(self, u, ag_updating, ag_updating_chained2):
        assert_equal(ag_updating_chained2.indices, ag_updating.indices)
        next(u.trajectory)
        assert_equal(ag_updating_chained2.indices, ag_updating.indices)
...
class TestUAGCallCount(object):
    ...
    def test_updated_when_next(self, u):
        ...
        with mock.patch.object(
            uag, "update_selection", wraps=uag.update_selection
        ) as mock_update:
            next(u.trajectory)
