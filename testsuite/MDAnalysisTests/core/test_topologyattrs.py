...
class TestAtomAttr(TopologyAttrMixin):
    ...
    def test_set_atoms_singular(self, attr):
        ...
        assert_equal(
            attr.get_atoms(dg), np.array([self.single_value, self.single_value])
        )
...
class AggregationMixin(TestAtomAttr):
    ...
    def test_get_segment(self, attr):
        assert_equal(
            attr.get_segments(DummyGroup(1)), np.sum(self.values[[4, 5, 8, 2, 3, 9]])
        )
...
class TestResidueAttr(TopologyAttrMixin):
    ...
    def test_get_residues(self, attr):
        assert_equal(
            attr.get_residues(DummyGroup([1, 2, 1, 3])), self.values[[1, 2, 1, 3]]
        )
...
class TestSegmentAttr(TopologyAttrMixin):
    ...
    def test_get_residues(self, attr):
        assert_equal(
            attr.get_residues(DummyGroup([1, 2, 1, 3])), self.values[[1, 1, 1, 0]]
        )
