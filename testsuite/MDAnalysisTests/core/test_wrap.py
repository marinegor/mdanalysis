...
class TestWrap(object):
    ...
    def test_unwrap_wrap_cycle(self, level, compound, center, is_triclinic):
        ...
        assert_almost_equal(
            group.atoms.positions, orig_unwrapped_pos, decimal=self.precision
        )
    ...
    def test_wrap_partial_compound(self, level, compound, center, is_triclinic):
        ...
        if level == "atoms":
            ...
            ref_wrapped_pos = np.concatenate(
                [ref_wrapped_pos[:-8], ref_wrapped_pos[-7:]]
            )
        elif level == "residues":
            group = group.residues
            ...
            ref_wrapped_pos = np.concatenate(
                [ref_wrapped_pos[:-8], ref_wrapped_pos[-4:]]
            )
        elif level == "segments":
            group = group.segments
            # molecule 12 missing
    ...
    def test_wrap_com_cog_difference(self, compound, is_triclinic):
        ...
        if compound == "atoms":
            ...
            shift[0] = 0.0
        assert_almost_equal(
            wrapped_pos_cog, wrapped_pos_com - shift, decimal=self.precision
        )
