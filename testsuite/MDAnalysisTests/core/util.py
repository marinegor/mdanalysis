...
class UnWrapUniverse(object):
    ...
    * Three linear chain molecules (type D) containing 8 atoms, spanning
    ...
    def __new__(
        ...
        # type A
        relpos[0:3, :] = np.array(
            [[0.5, 0.5, 0.5], [1.4, 0.5, 0.5], [2.1, 0.5, 0.5]], dtype=np.float32
        )
        # type B
    ...
    def unwrapped_coords(self, compound, reference):
        ...
        comp = compound.lower()
        if comp not in ["group", "segments", "residues", "molecules", "fragments"]:
        ...
        if reference is None:
            ...
            relpos[9:12, :] = np.array(
                [[0.8, 0.1, 1.05], [0.8, 0.1, 0.95], [0.9, 0.1, 0.95]], dtype=np.float32
            )
            # molecule 8
