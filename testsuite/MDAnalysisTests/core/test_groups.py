...
class TestEmptyAtomGroup(object):
    ...
    def test_passive_decorator(self, ag):
        assert_almost_equal(
            ag.center_of_mass(), np.array([10.52567673, 9.49548312, -8.15335145])
        )
        ...
        assert_almost_equal(
            ag.center_of_charge(), np.array([11.0800112, 8.8885659, -8.9886632])
        )
...
class TestGroupSlicing(object):
    ...
    group_dict = {"atom": u.atoms, "residue": u.residues, "segment": u.segments}
    singulars = {
        "atom": groups.Atom,
...
class TestComponentComparisons(object):
    ...
    @pytest.mark.parametrize(
        "x, y", itertools.permutations((u.atoms[0], u.residues[0], u.segments[0]), 2)
    )
...
class TestGroupBaseOperators(object):
    ...
    def groups(self, request, groups_simple, groups_duplicated_and_scrambled):
        return {"simple": groups_simple, "scrambled": groups_duplicated_and_scrambled}[
            request.param
        ]
...
class TestAtomGroup(object):
    def test_PDB_atom_repr(self):
        u = make_Universe(
            extras=("altLocs", "names", "types", "resnames", "resids", "segids")
        )
        assert_equal(
...
class TestAttributeGetting(object):
    ...
    @pytest.mark.parametrize(
        "attr,singular",
        [("elements", "element"), ("tempfactors", "tempfactor"), ("bonds", "bonds")],
    )
    ...
    def test_wrap_and_unwrap_deprecation(self, compound, pbc, unwrap):
        if pbc and unwrap:
            ...
            with pytest.warns(DeprecationWarning):
                ...
                # pylint: disable-next=unexpected-keyword-arg
                assert (
                    self.dummy_funtion(compound=compound, pbc=pbc, unwrap=unwrap) == 0
                )
