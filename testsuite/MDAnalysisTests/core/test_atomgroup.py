...
class TestAtomGroupToTopology(object):
    ...
    @pytest.mark.parametrize("btype,", ["bond", "angle", "dihedral", "improper"])
...
class _WriteAtoms(object):
    ...
    def test_write_atoms(self, universe, outfile, compression):
        ...
        assert_almost_equal(
            ...
            self.precision,
            err_msg=(
                "atom coordinate mismatch between original and {0!s} " "file".format(
                    self.ext + compression
                )
            ),
        )
    ...
    @pytest.mark.parametrize(
        "selection", ("name CA", "segid 4AKE and resname LEU", "segid 4AKE")
    )
    def test_write_selection(self, universe, outfile, selection):
        ...
        sel2 = u2.atoms
        assert len(u2.atoms) == len(sel.atoms), (
            "written selection does not " "match original selection"
        )
        assert_almost_equal(
            sel2.positions,
            sel.positions,
    ...
    def test_write_Residue(self, universe, outfile):
        G = (
            universe.select_atoms("segid 4AKE and resname ARG").residues[-2].atoms
        )  # 2nd to last Arg
        G.write(outfile)
        u2 = self.universe_from_tmp(outfile)
        # check EVERYTHING, otherwise we might get false positives!
        G2 = u2.atoms
        assert len(u2.atoms) == len(G.atoms), (
    ...
    def test_write_Universe(self, universe, outfile):
        ...
        with mda.Writer(outfile) as W:
            W.write(U)
        u2 = self.universe_from_tmp(outfile)
        assert len(u2.atoms) == len(U.atoms), (
...
class TestAtomGroupTransformations(object):
    ...
    def test_rotate(self, u, coords):
        ...
        for angle in np.linspace(0, np.pi):
            ...
            assert_almost_equal(ag.positions[0], [np.cos(angle), np.sin(angle), 0])
    ...
    def test_rotateby(self, u, coords):
        ...
        for angle in np.linspace(0, np.pi):
            ...
            assert_almost_equal(ag.positions[0], vec[0])
            assert_almost_equal(
                ag.positions[1], [-2 * np.cos(angle) + 1, -2 * np.sin(angle), 0]
            )
    ...
    def test_transform_translation_and_rotation(self, u):
        ...
        assert_almost_equal(ag.positions[0], [np.cos(angle) + 1, np.sin(angle) + 1, 1])
...
class TestSplit(object):
    ...
    def ag(self):
        ...
        return universe.select_atoms("resid 1:50 and not resname LYS and " "name CA CB")
...
class TestAtomGroupProperties(object):
    ...
    def get_new(att_type):
        ...
        if att_type == "string":
            ...
            return np.array(
                [0.001, 0.002, 0.003, 0.005, 0.012, 0.025], dtype=np.float32
            )
        elif att_type == "int":
            return [4, 6, 8, 1, 5, 4]
...
class TestUnwrapFlag(object):
    ...
    ref_noUnwrap = {
        "center_of_geometry": np.array([5.1, 7.5, 7.0], dtype=np.float32),
        "center_of_mass": np.array([6.48785, 7.5, 7.0], dtype=np.float32),
        "moment_of_inertia": np.array(
            [[0.0, 0.0, 0.0], [0.0, 98.6542, 0.0], [0.0, 0.0, 98.65421327]]
        ),
        "asphericity": 1.0,
        "shape_parameter": 1.0,
    }
    ...
    ref_Unwrap = {
        "center_of_geometry": np.array([10.1, 7.5, 7.0], dtype=np.float32),
        "center_of_mass": np.array([6.8616, 7.5, 7.0], dtype=np.float32),
        "moment_of_inertia": np.array(
            [[0.0, 0.0, 0.0], [0.0, 132.673, 0.0], [0.0, 0.0, 132.673]]
        ),
        "asphericity": 1.0,
        "shape_parameter": 1.0,
    }
    ...
    @pytest.mark.parametrize(
        "unwrap, ref", ((True, ref_Unwrap_residues), (False, ref_noUnwrap_residues))
    )
    @pytest.mark.parametrize(
        "method_name",
        (
    ...
    @pytest.mark.parametrize("unwrap, ref", ((True, ref_Unwrap), (False, ref_noUnwrap)))
    @pytest.mark.parametrize(
        "method_name",
        ("center_of_geometry", "center_of_mass", "moment_of_inertia", "asphericity"),
    )
...
class TestPBCFlag(object):
    ...
    ref_noPBC = {
        "center_of_geometry": np.array(
            [4.23789883, 0.62429816, 2.43123484], dtype=np.float32
        ),
        ...
        "bbox": np.array(
            [
                [-75.74159241, -144.86634827, -94.47974396],
    ...
    ref_PBC = {
        "center_of_geometry": np.array(
            [26.82960892, 31.5592289, 30.98238945], dtype=np.float32
        ),
        ...
        "bbox": np.array(
            [
                [0.145964116, 0.0185623169, 0.0431785583],
    ...
    @pytest.mark.parametrize("wrap, ref", ((True, ref_PBC), (False, ref_noPBC)))
    @pytest.mark.parametrize(
        "method_name",
        (
            "center_of_geometry",
...
class TestAtomGroup(object):
    ...
    def test_getitem_slice(self, universe):
        assert_equal(universe.atoms[0:4].ix, universe.atoms.ix[:4])

    def test_getitem_slice2(self, universe):
        assert_equal(universe.atoms[0:8:2].ix, universe.atoms.ix[0:8:2])
    ...
    def test_center_of_geometry(self, ag):
        assert_almost_equal(
            ag.center_of_geometry(), [-0.04223963, 0.0141824, -0.03505163], decimal=5
        )

    def test_center_of_mass(self, ag):
        assert_almost_equal(
            ag.center_of_mass(), [-0.01094035, 0.05727601, -0.12885778], decimal=5
        )
    ...
    @pytest.mark.parametrize("method_name", ("center_of_geometry", "center_of_mass"))
    @pytest.mark.parametrize(
        "name, compound", (("resids", "residues"), ("segids", "segments"))
    )
    ...
    def test_center_compounds_pbc(self, ag, name, compound, unwrap, method_name):
        ag.dimensions = [50, 50, 50, 90, 90, 90]
        ref = [
            getattr(a, method_name)(unwrap=unwrap) for a in ag.groupby(name).values()
        ]
        vals = getattr(ag, method_name)(compound=compound, unwrap=unwrap)
        assert_almost_equal(vals, ref, decimal=5)

    @pytest.mark.parametrize("method_name", ("center_of_geometry", "center_of_mass"))
    @pytest.mark.parametrize(
        "name, compound", (("molnums", "molecules"), ("fragindices", "fragments"))
    )
    def test_center_compounds_special(self, ag_molfrg, name, compound, method_name):
        ref = [getattr(a, method_name)() for a in ag_molfrg.groupby(name).values()]
    ...
    @pytest.mark.parametrize("unwrap", (True, False))
    def test_center_compounds_special_pbc(
        self, ag_molfrg, name, compound, unwrap, method_name
    ):
        ag_molfrg.dimensions = [50, 50, 50, 90, 90, 90]
        ref = [
            getattr(a, method_name)(unwrap=unwrap)
            for a in ag_molfrg.groupby(name).values()
        ]
        vals = getattr(ag_molfrg, method_name)(compound=compound, unwrap=unwrap)
        assert_almost_equal(vals, ref, decimal=5)
    ...
    @pytest.mark.parametrize("weights", (None, np.array([0.0]), np.array([2.0])))
    @pytest.mark.parametrize(
        "compound", ("group", "residues", "segments", "molecules", "fragments")
    )
    ...
    def test_center_compounds_zero_weights(self, ag_molfrg, wrap, name, compound):
        if compound == "group":
    ...
    def test_charges(self, ag):
        assert_almost_equal(
            ag.charges[1000:2000:200], np.array([-0.09, 0.09, -0.47, 0.51, 0.09])
        )
    ...
    def test_packintobox(self, universe):
        ...
        box = np.array([5.0, 5.0, 5.0, 90.0, 90.0, 90.0], dtype=np.float32)
        # Expected folded coordinates
        packed_coords = np.array(
            [
    ...
    def test_index_integer(self, universe):
        u = universe
        a = u.atoms[100]
        assert isinstance(a, mda.core.groups.Atom), (
    ...
    def test_index_slice(self, universe):
        ...
        assert isinstance(a, mda.core.groups.AtomGroup), (
    ...
    def test_index_advancedslice(self, universe):
        ...
        assert isinstance(ag, mda.core.groups.AtomGroup), (
            "advanced slicing " "does not produce " "an AtomGroup"
        )
        assert_equal(ag[1], ag[-1], "advanced slicing does not preserve order")

    def test_2d_indexing_caught(self, universe):
        u = universe
        index_2d = [[1, 2, 3], [4, 5, 6]]
        with pytest.raises(IndexError):
            u.atoms[index_2d]

    @pytest.mark.parametrize(
        "sel", (np.array([True, False, True]), [True, False, True])
    )
    ...
    def test_improper(self, universe):
        u = universe
        peptbond = u.select_atoms(
            "atom 4AKE 20 C", "atom 4AKE 21 CA", "atom 4AKE 21 N", "atom 4AKE 21 HN"
        )
        assert_almost_equal(
            peptbond.improper.value(),
            168.52952575683594,
            self.dih_prec,
    ...
    def test_dihedral_equals_improper(self, universe):
        ...
        assert_equal(
            peptbond.improper.value(),
            peptbond.dihedral.value(),
            "improper() and proper dihedral() give different results",
        )

    def test_bond(self, universe):
        sel2 = universe.select_atoms("segid 4AKE and resid 98").select_atoms(
            "name OE1", "name OE2"
        )
        assert_almost_equal(
            sel2.bond.value(), 2.1210737228393555, 3, "distance of Glu98 OE1--OE2 wrong"
        )
    ...
    def test_angle(self, universe):
        ...
        assert_almost_equal(
            sel3.angle.value(), 117.46187591552734, 3, "angle of Glu98 OE1-CD-OE2 wrong"
        )
    ...
    def test_atom_order(self, universe):
        assert_equal(universe.atoms.indices, sorted(universe.atoms.indices))
...
class TestAtomGroupSort(object):
    ...
    test_ids = ["ix", "ids", "resids", "segids", "charges", "masses", "tempfactors"]
    ...
    def test_sort_bonds(self, ag):
        with pytest.raises(ValueError, match=r"The array returned by the " "attribute"):
            ag.sort("bonds")
    ...
    def test_sort_position_no_keyfunc(self, ag):
        with pytest.raises(
            NameError,
            match=r"The .* attribute returns a "
