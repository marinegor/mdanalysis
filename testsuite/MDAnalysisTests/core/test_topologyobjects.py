...
class TestTopologyObjects(object):
    ...
    def test_dihedral_repr(self, PSFDCD):
        ...
        assert_equal(
            repr(dihedral), "<Dihedral between: Atom 4, Atom 7, Atom 8, Atom 1>"
        )
    ...
    def test_improper_repr(self, PSFDCD):
        ...
        assert_equal(
            repr(imp), "<ImproperDihedral between: Atom 4, Atom 7, Atom 8, Atom 1>"
        )
    ...
    def test_cmap_repr(self, PSFDCD):
        ...
        assert_equal(
            repr(cmap), "<CMap between: Atom 4, Atom 7, Atom 8, Atom 1, Atom 2>"
        )
...
class TestTopologyGroup_Cython(object):
    ...
    def test_right_type_bonds(self, bgroup, PSFDCD):
        assert_equal(
            bgroup.bonds(), calc_bonds(bgroup.atom1.positions, bgroup.atom2.positions)
        )
        assert_equal(
            bgroup.bonds(pbc=True),
            calc_bonds(
                bgroup.atom1.positions, bgroup.atom2.positions, box=PSFDCD.dimensions
            ),
        )
        assert_equal(
            bgroup.values(), calc_bonds(bgroup.atom1.positions, bgroup.atom2.positions)
        )
        assert_equal(
    ...
    def test_right_type_angles(self, agroup, PSFDCD):
        assert_equal(
            agroup.angles(),
            calc_angles(
                agroup.atom1.positions, agroup.atom2.positions, agroup.atom3.positions
            ),
        )
        assert_equal(
