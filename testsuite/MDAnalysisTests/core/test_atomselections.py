...
class TestSelectionsCHARMM(object):
    ...
    @pytest.mark.parametrize(
        "selstr", ["sphlayer 4.0 6.0 bynum 1281", "sphlayer 4.0 6.0 index 1280"]
    )
    ...
    @pytest.mark.parametrize(
        "selstr", ["isolayer 4.0 6.0 bynum 1281", "isolayer 4.0 6.0 index 1280"]
    )
    ...
    @pytest.mark.parametrize(
        "selstr",
        ["cylayer 4.0 6.0 10 -10 bynum 1281", "cylayer 4.0 6.0 10 -10 index 1280"],
    )
    ...
    @pytest.mark.parametrize(
        "selstr", ["cyzone 6.0 10 -10 bynum 1281", "cyzone 6.0 10 -10 index 1280"]
    )
    ...
    def test_concatenated_selection(self, universe):
        ...
        assert_equal(
            phi151[0].name, "HN", "wrong ordering in selection, should be HN-N-CA-CB"
        )
...
class TestSelectionsGRO(object):
    ...
    def test_same_coordinate(self, universe, selstr):
        ...
        assert_equal(
            sel.indices, target_ids, "Found wrong atoms with same x as ids 1 or 10"
        )

    @pytest.mark.parametrize(
        "selstr", ["cylayer 10 20 20 -20 bynum 3554", "cylayer 10 20 20 -20 index 3553"]
    )
    def test_cylayer(self, universe, selstr):
    ...
    @pytest.mark.parametrize(
        "selstr", ["cyzone 20 20 -20 bynum 3554", "cyzone 20 20 -20 index 3553"]
    )
    def test_cyzone(self, universe, selstr):
...
class BaseDistanceSelection(object):
    ...
    def test_isolayer(self, u, periodic):
        ...
        sel = Parser.parse(
            "isolayer {} {} (index {} or index {})".format(rmin, rmax, *r1.ix), u.atoms
        )
        if periodic:
            sel.periodic = True
        else:
    ...
    def test_point(self, u, periodic):
        ...
        box = u.dimensions if periodic else None
        d = distance_array(
            np.array([[5.0, 5.0, 5.0]], dtype=np.float32), u.atoms.positions, box=box
        )
...
class TestPropSelection(object):
    ...
    def test_lt(self, prop, selstr, ag):
        ...
        assert_equal(
            set(sel.indices), set(ag[getattr(ag, self.plurals[prop]) < 1.5].indices)
        )
