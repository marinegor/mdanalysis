...
@pytest.mark.parametrize(
    ...
    ((XTCFile, XTC_multi_frame, XTC_OFFSETS), (TRRFile, TRR_multi_frame, TRR_OFFSETS)),
    indirect=True,
)
...
def test_write_xtc_dtype(tmpdir, dtype, xtc):
    ...
    with XTCFile(fname, "w") as f:
        for frame in xtc:
            ...
            f.write(
                xyz=x, box=box, step=frame.step, time=frame.time, precision=frame.prec
            )
...
def test_write_different_box_trr(tmpdir, trr):
    ...
    with TRRFile(fname, "w") as f_out:
        ...
        box = frame.box.copy() + 1
        f_out.write(
            frame.x, frame.v, frame.f, box, frame.step, frame.time, frame.lmbda, natoms
        )
...
def test_write_different_x_trr(trr_writer):
    ...
    with pytest.raises(IOError):
        writer.write(
            x, frame.v, frame.f, frame.box, frame.step, frame.time, frame.lmbda, natoms
        )
...
def test_write_different_v(trr_writer):
    ...
    with pytest.raises(IOError):
        writer.write(
            frame.x, v, frame.f, frame.box, frame.step, frame.time, frame.lmbda, natoms
        )
...
def test_write_different_f(trr_writer):
    ...
    with pytest.raises(IOError):
        writer.write(
            frame.x, frame.v, f, frame.box, frame.step, frame.time, frame.lmbda, natoms
        )
