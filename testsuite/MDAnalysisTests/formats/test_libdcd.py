...
@pytest.mark.parametrize(
    "dcdfile, is_periodic",
    [(DCD, False), (DCD_NAMD_TRICLINIC, True), (DCD_TRICLINIC, True)],
)
...
@pytest.mark.parametrize(
    "dcdfile, natoms", [(DCD, 3341), (DCD_NAMD_TRICLINIC, 5545), (DCD_TRICLINIC, 375)]
)
...
@pytest.mark.parametrize(
    "dcdfile, unit_cell",
    [
        (DCD, [0.0, 90.0, 0.0, 90.0, 90.0, 0.0]),
        (DCD_NAMD_TRICLINIC, [38.42659378, 0.499563, 38.393102, 0.0, 0.0, 44.7598]),
        (
            DCD_TRICLINIC,
...
@pytest.mark.parametrize(
    "dcdfile, nframes", [(DCD, 98), (DCD_NAMD_TRICLINIC, 1), (DCD_TRICLINIC, 10)]
)
...
DCD_NAMD_TRICLINIC_HEADER = (
    "Created by DCD pluginREMARKS Created 06 July, 2014 at 17:29Y5~CORD,"
)
...
@pytest.mark.parametrize(
    ...
    (
        (DCD, DCD_HEADER),
        (DCD_NAMD_TRICLINIC, DCD_NAMD_TRICLINIC_HEADER),
...
def test_write_header(tmpdir):
    ...
    with DCDFile(testfile, "w") as dcd:
        dcd.write_header(
            remarks="Crazy!", natoms=22, istart=12, nsavc=10, delta=0.02, is_periodic=1
        )
    ...
    with DCDFile(testfile) as dcd:
        ...
        assert np.allclose(header["delta"], 0.02)
...
def test_written_num_frames(written_dcd):
    with DCDFile(written_dcd.testfile) as dcd, DCDFile(written_dcd.orgfile) as other:
...
def test_written_coord_match(written_dcd):
    with DCDFile(written_dcd.testfile) as test, DCDFile(written_dcd.orgfile) as ref:
...
def test_write_all_dtypes(tmpdir, dtype):
    ...
    with DCDFile(fname, "w") as out:
        ...
        out.write_header(
            remarks="test", natoms=natoms, is_periodic=1, delta=1, nsavc=1, istart=1
        )
