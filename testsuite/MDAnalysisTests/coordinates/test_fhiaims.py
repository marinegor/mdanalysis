...
class TestFHIAIMSReader(object):
    ...
    def test_single_file(self, universe, universe_from_one_file):
        assert_almost_equal(
            ...
            universe_from_one_file.atoms.positions,
            self.prec,
            "FHIAIMSReader failed to load universe from single file",
        )
    ...
    def test_uses_FHIAIMSReader(self, universe):
        ...
        assert isinstance(
            universe.trajectory, FHIAIMSReader
        ), "failed to choose FHIAIMSReader"
    ...
    def test_dimensions(self, ref, universe):
        assert_almost_equal(
            ...
            universe.dimensions,
            self.prec,
            "FHIAIMSReader failed to get unitcell dimensions",
        )
    ...
    def test_n_frames(self, ref, universe):
        assert_equal(
            ref.n_frames, universe.trajectory.n_frames, "wrong number of frames"
        )
    ...
    def test_bad_input_relative_positions_no_box(
        self, bad_input_relative_positions_no_box
    ):
        with pytest.raises(
            ValueError,
