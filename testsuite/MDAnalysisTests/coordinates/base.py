...
class _SingleFrameReader(TestCase, RefAdKSmall):
    ...
    def test_n_atoms(self):
        assert_equal(
            self.universe.trajectory.n_atoms, self.ref_n_atoms, "wrong number of atoms"
        )
    ...
    def test_n_frames(self):
        assert_equal(
            self.universe.trajectory.n_frames, 1, "wrong number of frames in pdb"
        )
    ...
    def test_pickle_singleframe_reader(self):
        ...
        assert_equal(
            reader.ts, reader_p.ts, "Single-frame timestep is changed after pickling"
        )
...
class BaseReaderTest(object):
    ...
    def reader(ref):
        ...
        reader.add_auxiliary(
            "lowf", ref.aux_lowf, dt=ref.aux_lowf_dt, initial_time=0, time_selector=None
        )
        reader.add_auxiliary(
            "highf",
            ref.aux_highf,
    ...
    def test_changing_dimensions(self, ref, reader):
        if ref.changing_dimensions:
            ...
            if ref.dimensions_second_frame is None:
                ...
                assert_array_almost_equal(
                    reader.ts.dimensions, ref.dimensions_second_frame, decimal=ref.prec
                )
    ...
    def test_get_aux_attribute(self, ref, reader):
        assert_equal(reader.get_aux_attribute("lowf", "dt"), ref.aux_lowf_dt)
    ...
    def test_reload_auxiliaries_from_description(self, ref, reader):
        ...
        assert_equal(
            reader.aux_list, reader.aux_list, "Number of auxiliaries does not match"
        )
        # each auxiliary should be the same
        for auxname in reader.aux_list:
            assert_equal(
                reader._auxs[auxname], reader._auxs[auxname], "AuxReaders do not match"
            )
    ...
    def test_pickle_reader(self, reader):
        ...
        assert_equal(len(reader), len(reader_p_p))
        assert_equal(
            reader.ts, reader_p_p.ts, "Timestep is changed after double pickling"
        )
        reader.ts.positions[0] = np.array([1, 2, 3])
        reader_p = pickle.loads(pickle.dumps(reader))
        assert_equal(
    ...
    def test_timeseries_empty_asel(self, reader):
        with pytest.warns(
            UserWarning, match="Empty string to select atoms, empty group returned."
        ):
...
class MultiframeReaderTest(BaseReaderTest):
    ...
    def test_reopen(self, ref, reader):
        ...
        reader._reopen()
        ts = reader.next()
        assert_timestep_almost_equal(ts, ref.first_frame, decimal=ref.prec)

    def test_rename_aux(self, ref, reader):
        reader.rename_aux("lowf", "lowf_renamed")
        # data should now be in aux namespace under new name
        assert_equal(reader.ts.aux.lowf_renamed, ref.aux_lowf_data[0])
        # old name should be removed
        with pytest.raises(AttributeError):
            getattr(reader.ts.aux, "lowf")
        # new name should be retained
        next(reader)
        assert_equal(reader.ts.aux.lowf_renamed, ref.aux_lowf_data[1])
    ...
    def test_iter_as_aux_lowf(self, ref, reader):
        ...
        for i, ts in enumerate(reader.iter_as_aux("lowf")):
            assert_timestep_almost_equal(
                ts, ref.iter_ts(ref.aux_lowf_frames_with_steps[i]), decimal=ref.prec
            )

    @pytest.mark.parametrize(
        "accessor",
        [lambda traj: traj[[0, 1, 2]], lambda traj: traj[:3], lambda traj: traj],
    ...
    def test_pickle_next_ts_reader(self, reader):
        reader_p = pickle.loads(pickle.dumps(reader))
        assert_equal(
            next(reader), next(reader_p), "Next timestep is changed after pickling"
        )
        reader_p_p = pickle.loads(pickle.dumps(reader_p))
        assert_equal(
            next(reader),
    ...
    def test_pickle_last_ts_reader(self, reader):
        ...
        reader_p = pickle.loads(pickle.dumps(reader))
        assert_equal(
            len(reader), len(reader_p), "Last timestep is changed after pickling"
        )
...
class BaseWriterTest(object):
    ...
    def test_write_selection(
        self, ref, reader, universe, u_no_resnames, u_no_resids, u_no_names, tmpdir
    ):
        sel_str = "resid 1"
    ...
    def _check_copy(self, fname, ref, reader):
        ...
        for orig_ts, copy_ts in zip(reader, copy):
            assert_timestep_almost_equal(copy_ts, orig_ts, decimal=ref.prec)
...
def assert_timestep_almost_equal(A, B, decimal=6, verbose=True):
    ...
    if A.frame != B.frame:
        raise AssertionError(
            "A and B refer to different frames: " "A.frame = {}, B.frame={}".format(
                A.frame, B.frame
            )
        )
    ...
    if not np.allclose(A.time, B.time):
        raise AssertionError(
            "A and B refer to different times: " "A.time = {}, B.time={}".format(
                A.time, B.time
            )
        )
    ...
    if A.n_atoms != B.n_atoms:
        raise AssertionError(
            "A and B have a differnent number of atoms: "
            "A.n_atoms = {}, B.n_atoms = {}".format(A.n_atoms, B.n_atoms)
        )

    if A.has_positions != B.has_positions:
        raise AssertionError(
            "Only one Timestep has positions:"
    ...
    if A.has_velocities != B.has_velocities:
        raise AssertionError(
            "Only one Timestep has velocities:"
    ...
    if len(A.aux) > 0 and len(B.aux) > 0:
        assert_equal(
            A.aux,
            B.aux,
            err_msg="Auxiliary values do not match: " "A.aux = {}, B.aux = {}".format(
                A.aux, B.aux
            ),
        )
