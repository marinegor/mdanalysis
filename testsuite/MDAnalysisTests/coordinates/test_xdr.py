...
class _GromacsReader(object):
    ...
    def test_jump_lastframe_xdrtrj(self, universe):
        ...
        assert_equal(universe.coord.frame, 9, "indexing last frame with trajectory[-1]")
    ...
    def test_volume(self, universe):
        ...
        assert_almost_equal(
            vol, self.ref_volume, 0, err_msg="unit cell volume (rhombic dodecahedron)"
        )
    ...
    def test_Writer(self, tmpdir):
        ...
        # 3...
        assert_almost_equal(u.atoms.positions, universe.atoms.positions, self.prec)
...
class TestXTCReaderClass(object):
    def test_with_statement(self):
        ...
        assert_equal(
            N, 10, err_msg="with_statement: XTCReader reads wrong number of frames"
        )
        assert_equal(
            frames,
            np.arange(0, N),
...
class _GromacsWriter(object):
    ...
    def test_timestep_not_modified_by_writer(self, universe, Writer, outfile):
        ...
        assert_equal(
            ts._pos, x, err_msg="Positions in Timestep were modified by writer."
        )
...
class TestTRRWriter(_GromacsWriter):
    ...
    def test_data_preservation(self, universe, Writer, outfile):
        ...
        for orig_ts, written_ts in zip(universe.trajectory, uw.trajectory):
            ...
            for k in orig_ts.data:
                assert_allclose(orig_ts.data[k], written_ts.data[k], err_msg=err_msg)
...
class _GromacsReader_offsets(object):
    ...
    def test_offsets(self, trajectory, traj):
        ...
        assert_almost_equal(
            trajectory._xdr.offsets, self.ref_offsets, err_msg="wrong frame offsets"
        )
    ...
    def test_nonexistent_offsets_file(self, traj):
        ...
        with patch.object(np, "load") as np_load_mock:
            ...
            with pytest.warns(
                UserWarning,
                match=re.escape(f"Failed to load offsets file {outfile_offsets}"),
            ):
                saved_offsets = XDR.read_numpy_offsets(outfile_offsets)
    ...
    def test_persistent_offsets_readonly(self, tmpdir):
        ...
        with pytest.warns(UserWarning, match="Couldn't save offsets") and pytest.warns(
            UserWarning, match="Cannot write"
        ):
        ...
        assert_equal(
            os.path.exists(XDR.offsets_filename(filename, ending=".lock")), False
        )
...
class TestXTCReader_offsets(_GromacsReader_offsets):
    ...
    ref_offsets = np.array(
        [0, 165188, 330364, 495520, 660708, 825872, 991044, 1156212, 1321384, 1486544]
    )
