...
class TestH5MDReaderWithRealTrajectory(object):
    ...
    @pytest.mark.parametrize(
        "group, attr",
        (("position", "positions"), ("velocity", "velocities"), ("force", "forces")),
    )
    ...
    @pytest.mark.parametrize(
        "dset", ("position/value", "position/time", "velocity/value", "force/value")
    )
    ...
    def test_wrong_driver(self):
        with pytest.raises(
            ...
            u = mda.Universe(
                TPR_xvf, H5MD_xvf, driver="wrong_driver", comm="mock MPI.COMM_WORLD"
            )
    ...
    @pytest.mark.parametrize(
        "group1, group2",
        (("velocity", "force"), ("position", "force"), ("position", "velocity")),
    )
...
class TestH5MDWriterWithRealTrajectory(object):
    ...
    def test_write_custom_units(
        self, universe, outfile, Writer, timeunit, lengthunit, velocityunit, forceunit
    ):
        with Writer(
            outfile,
            universe.atoms.n_atoms,
        ...
        for u_unit, custom_unit in zip(
            u.trajectory.units.values(), (timeunit, lengthunit, velocityunit, forceunit)
        ):
    ...
    def test_write_with_compression(self, universe, outfile, Writer, filter, opts):
        with Writer(
            outfile, universe.atoms.n_atoms, compression=filter, compression_opts=opts
        ) as W:
            for ts in universe.trajectory:
                W.write(universe)
