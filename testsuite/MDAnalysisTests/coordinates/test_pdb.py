...
class TestPDBMetadata(object):
    ...
    def test_TITLE(self, universe):
        ...
        for lineno, (parsed, reference) in enumerate(zip(title, self.title), start=1):
            assert_equal(
                parsed, reference, err_msg="TITLE line {0} do not match".format(lineno)
            )
    ...
    def test_COMPND(self, universe):
        ...
        for lineno, (parsed, reference) in enumerate(
            ...
            assert_equal(
                parsed, reference, err_msg="COMPND line {0} do not match".format(lineno)
            )
    ...
    def test_REMARK(self, universe):
        ...
        for lineno, (parsed, reference) in enumerate(
            ...
            assert_equal(
                parsed, reference, err_msg="REMARK line {0} do not match".format(lineno)
            )
...
class TestPDBWriter(object):
    ...
    def test_write_single_frame_Writer(self, universe2, outfile):
        ...
        assert_equal(
            u2.trajectory.n_frames, 1, err_msg="The number of frames should be 1."
        )
...
class TestMultiPDBReader(object):
    ...
    def test_rewind(self, multiverse):
        ...
        u.trajectory.rewind()
        assert_equal(
            u.trajectory.ts.frame, 0, "Failed to rewind to 0th frame (frame index 0)"
        )
...
class TestMultiPDBWriter(object):
    ...
    def test_write_atoms(self, universe2, outfile):
        ...
        assert_equal(
            u0.trajectory.n_frames, 2, err_msg="The number of frames should be 2."
        )
...
class TestPDBReaderBig(RefAdK):
    ...
    def test_selection(self, universe):
        ...
        assert_equal(
            len(na), self.ref_Na_sel_size, "Atom selection of last atoms in file"
        )

    def test_n_atoms(self, universe):
        assert_equal(
            universe.trajectory.n_atoms, self.ref_n_atoms, "wrong number of atoms"
        )
...
@pytest.mark.parametrize(
    "atom, refname",
    (
        (mda.coordinates.PDB.Pair("ASP", "CA"), " CA "),  # Regular protein carbon alpha
