...
def test_unwrap_vel_force():
    ...
    assert_allclose(
        u_wrapped.atoms.positions[0], np.array([2.56616, 6.11565, 7.37956]), atol=1e-5
    )
...
def test_unwrap_image_wrap():
    u_unwrapped = mda.Universe(
        LAMMPS_image_vf, LAMMPSDUMP_image_vf, format="LAMMPSDUMP", unwrap_images=True
    )
...
class TestLAMMPSDCDReader(RefLAMMPSDataDCD):
    ...
    def test_dt(self, u):
        assert_allclose(
            u.trajectory.dt, self.dt, err_msg="Time between frames dt is wrong."
        )
    ...
    def test_LAMMPSDCDReader_set_dt(self, u, dt=1500.0):
        ...
        assert_allclose(
            u.trajectory[iframe].time,
            iframe * dt,
            err_msg="setting time step dt={0} failed: " "actually used dt={1}".format(
    ...
    def test_wrong_time_unit(self):
        def wrong_load(unit="nm"):
            return mda.Universe(
                self.topology, self.trajectory, format=self.format, timeunit=unit
            )
...
class TestLAMMPSDCDWriter(RefLAMMPSDataDCD):
    ...
    def test_Writer(self, u, tmpdir, n_frames=3):
        ...
        assert_equal(
            short.trajectory.n_frames, n_frames, err_msg="number of frames mismatch"
        )
        assert_allclose(
    ...
    def test_OtherWriter(self, u, tmpdir):
        ...
        assert_allclose(
            rev_times, times[::-1], 6, err_msg="time steps of written DCD mismatch"
        )
        assert_allclose(
...
def test_open_absent_convention_fails(convention):
    with pytest.raises(ValueError, match="No coordinates following"):
        mda.Universe(
            LAMMPSDUMP, format="LAMMPSDUMP", lammps_coordinate_convention=convention
        )
...
class TestCoordinateMatches(object):
    ...
    def reference_unscaled_positions(self):
        ...
        return np.asarray(
            [atom340_pos1_unscaled, atom340_pos2_unscaled, atom340_pos3_unscaled]
        )
    ...
    def test_unscaled_reference(self, universes, reference_unscaled_positions):
        ...
        for i, ts_u in enumerate(universes["unscaled"].trajectory[0:3]):
            assert_allclose(
                atom_340.position, reference_unscaled_positions[i, :], atol=1e-5
            )
    ...
    def test_scaled_reference(self, universes, reference_unscaled_positions):
        ...
        for i, ts_u in enumerate(universes["scaled"].trajectory[0:3]):
            assert_allclose(
                atom_340.position, reference_unscaled_positions[i, :], atol=1e-1
            )
    ...
    def reference_unwrapped_positions(self):
        ...
        return np.asarray(
            [atom340_pos1_unwrapped, atom340_pos2_unwrapped, atom340_pos3_unwrapped]
        )
    ...
    def test_unwrapped_scaled_reference(self, universes, reference_unwrapped_positions):
        ...
        for i, ts_u in enumerate(universes["unwrapped"].trajectory[0:3]):
            assert_allclose(
                atom_340.position, reference_unwrapped_positions[i, :], atol=1e-5
            )
    ...
    def test_unwrapped_scaled_reference(self, universes, reference_unwrapped_positions):
        ...
        for i, ts_u in enumerate(universes["scaled_unwrapped"].trajectory[0:3]):
            assert_allclose(
                atom_340.position, reference_unwrapped_positions[i, :], atol=1e-1
            )
            # NOTE this seems a bit inaccurate?
    ...
    def test_unwrapped_scaled_unwrapped_match(self, universes):
        ...
        for ts_u, ts_s in zip(
            universes["unwrapped"].trajectory, universes["scaled_unwrapped"].trajectory
        ):
            assert_allclose(ts_u.positions, ts_s.positions, atol=1e-1)
            # NOTE this seems a bit inaccurate?
