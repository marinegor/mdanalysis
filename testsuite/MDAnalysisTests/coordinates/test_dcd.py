...
class TestDCDReader(MultiframeReaderTest):
    ...
    def test_with_statement(self):
        ...
        assert_equal(
            N, 98, err_msg="with_statement: DCDReader reads wrong number of frames"
        )
        assert_array_equal(
            frames,
            np.arange(0, N),
...
def test_single_frame(universe_dcd, tmpdir):
    ...
    assert_almost_equal(
        w.atoms.positions, u.atoms.positions, 3, err_msg="coordinates do not match"
    )
...
class RefNAMDtriclinicDCD(object):
    ...
    ref_dimensions = np.array(
        [
            [1.0, 38.426594, 38.393101, 44.759800, 90.000000, 90.000000, 60.028915],
        ]
    )
    ...
    with tmpdir.as_cwd():
        ...
        for ts_orig, ts_copy in zip(u.trajectory, w.trajectory):
            assert_almost_equal(
                ...
                ts_copy.dimensions,
                4,
                err_msg="DCD->DCD: unit cell dimensions wrong " "at frame {0}".format(
                    ts_orig.frame
                ),
            )
