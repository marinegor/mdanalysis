...
class AuxStep(object):
    ...
    def _time_selector(self):
        ...
        try:
            ...
            warnings.warn(
                "{} does not support time selection. Reverting to " "default".format(
    ...
    def _data_selector(self):
        ...
        try:
            ...
            warnings.warn(
                "{} does not support data selection. Reverting to " "default".format(
                    self.__class__.__name__
                )
            )
            return None
    ...
    **kwargs
    ...
    def _check_index(self, i):
        ...
        if i < 0 or i >= self.n_steps:
            raise IndexError(
                "{} is out of range of auxiliary (num. steps " "{})".format(
                    i, self.n_steps
                )
            )
        return i
...
class AuxFileReader(AuxReader):
    ...
    **kwargs
    ...
    def _go_to_step(self, i):
        ...
        if i >= self.n_steps:
            raise ValueError(
                "Step index {0} is not valid for auxiliary " "(num. steps {1}!".format(
                    i, self.n_steps
                )
            )
        value = self.rewind()
