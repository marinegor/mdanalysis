...
class TopologyGroup(object):
    ...
    def __init__(
        self, bondidx, universe, btype=None, type=None, guessed=None, order=None
    ):
        if btype is None:
            # guess what I am
            ...
            raise ValueError(
                "Unsupported btype, use one of '{}'" "".format(
                    ", ".join(_BTYPE_TO_SHAPE)
                )
            )
    ...
    def dihedrals(self, result=None, pbc=False):
        ...
        """
        return self._calc_connection_values(
            distances.calc_dihedrals, "dihedral", "improper", pbc=pbc, result=result
        )
