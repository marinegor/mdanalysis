...
class _TopologyAttrContainer(object):
    ...
    * the mixing (subclassing and co-inheritance) with the container classes.
    ...
    def _add_prop(cls, attr):
        ...
        if cls._is_group:
            setattr(cls, attr.attrname, property(getter, setter, None, attr.groupdoc))
            cls._SETATTR_WHITELIST.add(attr.attrname)
        else:
            setattr(cls, attr.singular, property(getter, setter, None, attr.singledoc))
    ...
    def __getattr__(self, attr):
        ...
        if attr in _TOPOLOGY_TRANSPLANTS:
            ...
            if not isinstance(self, clstype):
                ...
                raise AttributeError(
                    err.format(
                        attr=attrname, method=attrtype, clstype=clsname, selfcls=selfcls
                    )
                )
...
def _only_same_level(function):
    ...
    def wrapped(self, other):
        if not isinstance(other, (ComponentBase, GroupBase)):  # sanity check
            raise TypeError(
                "Can't perform '{}' between objects:" " '{}' and '{}'".format(
                    function.__name__, type(self).__name__, type(other).__name__
                )
            )
        if self.level != other.level:
            raise TypeError(
                "Can't perform '{}' on different level objects" "".format(
                    function.__name__
                )
            )
...
class GroupBase(_MutableBase):
    ...
    def __str__(self):
        ...
        if len(self) <= 10:
            ...
            return "<{}Group {}, ..., {}>".format(
                name.capitalize(), repr(list(self)[:3])[:-1], repr(list(self)[-3:])[1:]
            )
            ...
            for atom_mask in atom_masks:
                ...
                if reference is not None:
                    if reference == "com":
                        ...
                        refpos = np.einsum(
                            "ijk,ijk->ik", positions[atom_mask], masses[:, :, None]
                        )
...
class AtomGroup(GroupBase):
    ...
    def residues(self, new):
        ...
        if not isinstance(r_ix, itertools.cycle) and len(r_ix) != len(self):
            raise ValueError(
                "Incorrect size: {} for AtomGroup of size: {}" "".format(
                    len(new), len(self)
                )
            )
        # Optimisation TODO:
    ...
    def write(
        ...
        **kwargs,
        ...
        filename = util.filename(
            filename, ext=file_format if file_format is not None else "PDB", keep=True
        )
...
class ResidueGroup(GroupBase):
    ...
    def segments(self, new):
        ...
        if not isinstance(s_ix, itertools.cycle) and len(s_ix) != len(self):
            raise ValueError(
                "Incorrect size: {} for ResidueGroup of size: {}" "".format(
                    len(new), len(self)
                )
            )
        # Optimisation TODO:
                    ...
                    if missing:
                        raise NoDataError(
                            ...
                            "attributes: {attrs}".format(
                                funcname=func.__name__, attrs=", ".join(missing)
                            )
                        )
