...
class WaterBridgeAnalysis(AnalysisBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.selection1_type not in ("both", "donor", "acceptor"):
            raise ValueError(
                "WaterBridgeAnalysis: " "Invalid selection type {0!s}".format(
    ...
    def _log_parameters(self):
        ...
        logger.info(
            "WaterBridgeAnalysis: criterion: " "angle D-H-A >= %.3f degrees", self.angle
        )
        logger.info(
    ...
    def _update_selection(self):
        ...
        if self.filter_first and len(self._s1):
            self.logger_debug(
                "Size of selection 1 before filtering:" " {} atoms".format(
                    len(self._s1)
                )
            )
        ...
        if self.filter_first and len(self._s2):
            self.logger_debug(
                "Size of selection 2 before filtering:" " {} atoms".format(
                    len(self._s2)
                )
            )
    ...
    def _update_water_selection(self):
        ...
        self.logger_debug(
            "Size of water selection before filtering:" " {} atoms".format(
                len(self._water)
            )
        )
    ...
    def _get_bonded_hydrogens(self, atom):
        ...
        try:
            return atom.residue.atoms.select_atoms(
                "(name H* 1H* 2H* 3H* or type H) and around {0:f} name {1!s}" "".format(
    ...
    def _single_frame(self):
        ...
        if self.selection1_type in ("donor", "both"):
            ...
            if self.order > 0:
                ...
                results = self._donor2acceptor(
                    self._water_donors_h, self._water_h_donors, self._s2_acceptors
                )
                for line in results:
                    (
                        h_index,
        ...
        if self.selection1_type in ("acceptor", "both"):
            ...
            if self.order > 0:
                ...
                results = self._donor2acceptor(
                    self._water_donors_h, self._water_h_donors, self._s1_acceptors
                )
                for line in results:
                    (
                        h_index,
        ...
        if self.order > 1:
            ...
            results = self._donor2acceptor(
                self._water_donors_h, self._water_h_donors, self._water_acceptors
            )
            for line in results:
                (
                    h_index,
    ...
    def _traverse_water_network(
        self, graph, current, analysis_func=None, output=None, link_func=None, **kwargs
    ):
        """
            ...
            if len(current) <= self.order:
                for node in graph:
                    ...
                    self._traverse_water_network(
                        graph[node], new, analysis_func, output, link_func, **kwargs
                    )
    ...
    def generate_table(self, output_format=None):
        ...
        logger.debug(
            "WBridge: Stored results as table with %(num_records)d entries.", vars()
        )
        self.table = table
