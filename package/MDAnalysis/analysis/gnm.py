...
class GNMAnalysis(AnalysisBase):
        ...
        if ReportVector:
            with open(ReportVector, "a") as oup:
                for item in enumerate(v[list_map[1]]):
                    print("", counter, item[0] + 1, w[list_map[1]], item[1], file=oup)
    ...
    def _single_frame(self):
        ...
        self._generate_output(
            w, v, self.results, ReportVector=self.ReportVector, counter=self._ts.frame
        )
...
class closeContactGNMAnalysis(GNMAnalysis):
    ...
    def __init__(
        self, universe, select="protein", cutoff=4.5, ReportVector=None, weights="size"
    ):
