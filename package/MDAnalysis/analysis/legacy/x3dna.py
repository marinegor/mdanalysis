...
class X3DNA(BaseX3DNA):
    ...
    def run(self, **kwargs):
        ...
        if x3dna.bit_length != 0:
            logger.fatal(
                "X3DNA Failure (%d). Check output %r", x3dna.bit_length, outname
            )
    ...
    def collect(self, **kwargs):
        ...
        logger.info(
            "Run %s: Reading %d X3DNA profiles from %r", run, length, x3dna_output
        )
        x3dna_profile_no = 0
        records = []
        with open(x3dna_output, "r") as x3dna:
            ...
            for line in x3dna:
                ...
                if self.x3dna_param is False:
                    ...
                    if read_data:
                        if len(line.strip()) != 0:
                            try:
                                Sequence, Shift, Slide, Rise, Tilt, Roll, Twist = (
                                    line.split()
                                )
                            except:
                                logger.critical(
            ...
            if outdir is not None:
                ...
                frame_x3dna_txt = os.path.join(
                    rundir, "bp_step_{0!s}_{1:04d}.dat.gz".format(run, x3dna_profile_no)
                )
