    ...
    def reset(self):
        ...
        try:
            ...
            try:
                ...
                warnings.warn(
                    "NamedStream {0}: not guaranteed to be at the beginning." "".format(
                        self.name
                    ),
                    category=StreamWarning,
                )
        ...
        if len(np.asarray(weights, dtype=object).shape) != 1:
            raise ValueError(
                "weights must be a 1D array, not with shape " "{0}".format(
                    np.asarray(weights, dtype=object).shape
                )
            )
...
def blocks_of(a, n, m):
    ...
    new_strides = (n * a.strides[0] + m * a.strides[1], a.strides[0], a.strides[1])
...
class _Deprecate(object):
    ...
    def __init__(
        self, old_name=None, new_name=None, release=None, remove=None, message=None
    ):
    ...
    def __call__(self, func, *args, **kwargs):
        ...
        .. deprecated:: {0}
           {1}
           {2}
        """.format(release, message if message else depdoc, remove_text)
        )
