...
def _check_result_array(result: Optional[npt.NDArray], shape: tuple) -> npt.NDArray:
    ...
    if result.shape != shape:
        raise ValueError(
            "Result array has incorrect shape, should be {0}, got " "{1}.".format(
                shape, result.shape
            )
        )
    if result.dtype != np.float64:
        raise TypeError(
            "Result array must be of type numpy.float64, got {}." "".format(
                result.dtype
            )
        )
...
def self_capped_distance(
    ...
    function = _determine_method_self(
        reference_positions, max_cutoff, min_cutoff=min_cutoff, box=box, method=method
    )
    return function(
        reference,
        max_cutoff,
...
@check_coords(
    "reference", enforce_copy=False, reduce_result_if_single=False, allow_atomgroup=True
)
...
def calc_angles(
    ...
    if numatom > 0:
        if box is not None:
            ...
            _run(
                "calc_angle", args=(coords1, coords2, coords3, angles), backend=backend
            )
